...
public class MessageFileRcvAdapter extends BaseRcvAdapter {
    ...
    protected void bindViewData(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof ItemViewHolder && !fileInfoList.isEmpty()) {
            ...
            iHolder.detailTv.setText(StrUtil.formatFileLength(fileInfoList.get(position).getFileLength())
            ...
            iHolder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    iHolder.selectIb.setSelected(!iHolder.selectIb.isSelected());
                    if (iHolder.selectIb.isSelected()) {
                        if (selectList.size() < 9) {
                            selectList.add(fileInfoList.get(position));
                        } else {
                            iHolder.selectIb.setSelected(false);
    ...
    private static class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        public ItemViewHolder(View itemView) {
            super(itemView);
            selectIb = itemView.findViewById(R.id.ib_message_chat_file_document_item_check);
