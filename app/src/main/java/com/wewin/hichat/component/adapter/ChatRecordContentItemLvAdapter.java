 ...
 */
public class ChatRecordContentItemLvAdapter extends BaseAdapter {
    ...
    ChatRecordContentItemLvAdapter(Context context, List<ChatRecordSearch.ContentItem> contentItemList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ((lvExpand && position < contentItemList.size()) || (position < contentItemList.size() && position < 3)) {
            ...
            if (TimeUtil.isSameDay(TimeUtil.getServerTimestamp(), contentItemList.get(position).getCreateTimestamp())) {
                iHolder.timeTv.setText(TimeUtil.timestampToStr(contentItemList.get(position).getCreateTimestamp(), "HH:mm"));
            } else {
                iHolder.timeTv.setText(TimeUtil.getFormatDate(contentItemList.get(position).getCreateTimestamp()));
            }
        }
